...
@connect(({ _discussion, loading }) => {
  return {
    _discussion,
    loading: loading.models._discussion,
  };
})
...
class DiscussionList extends PureComponent {
    ...
    columns: [
      ...
      },
      {
        ...
        render: (record) => {
          return (
            <Fragment>
              <a onClick={() => this.previewItem(record.discussionId)}>配置</a>
   ...
   */
  previewItem = id => {
    router.push(`/discussion/discussionForm/${id}`);
  };
  newItem = () => {
    router.push(`/discussion/discussionForm`);
  };
  ...
  handleMenuClick = e => {
    ...
    switch (e.key) {
      case 'remove':
        dispatch({
          type: '_discussion/remove',
          payload: {
            discussionIds: selectedRows.map(row => row.discussionId).join(','),
          },
          callback: () => {
            this.setState({
              selectedRows: [],
  ...
  renderAdvancedForm() {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <FormItem label="日常活动ID">
              {getFieldDecorator('routineId')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>       
          <Col md={8} sm={24}>
          ...
          <Col md={8} sm={24}>
            <FormItem label="发布者ID">
              {getFieldDecorator('routineUserId')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>       
        </Row>
